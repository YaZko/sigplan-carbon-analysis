...
class Place:
    ...
    def __eq__(self, other):
        if self.state is None and other.state is None:
            ...
            return (self.city, self.state, self.country) == (other.city, other.state,
                                                             other.country)
...
class Location:
    ...
    def csv(self):
        return "{},{},{},{},{},{}".format(self.place.city, self.place.state, self.place.country,
                                          self.country_iso, self.continent, self.GPS, self.airport)

    def __repr__(self):
        return "Location: ({},{},{},{},{},{})".format(self.place.city, self.place.state,
                                                      self.place.country, self.country_iso,
                                                      self.continent, self.GPS, self.airport)
    ...
    def get_and_set_GPS(self, GLOB, cache):
        if self.GPS is None:
            if self.place not in cache.cache:
    ...
    def get_continent(self, GLOB, iso, gps):
        ...
        try:
            ...
            print("Failed to find the continent associated to country {}".format(
                str(e).split(':')[1].strip()))
            return None
    ...
    def write_csv_row(self, writer):
        writer.writerow([
            self.place.city, self.place.state, self.place.country, self.country_iso,
            self.continent, self.GPS, self.airport
        ])
...
class RawData:
    ...
    def csv(self):
        return "{},{},{},{},{},{}".format(self.id, self.location.place.city,
                                          self.location.place.state, self.location.place.country,
                                          self.conference, self.year)

    def __repr__(self):
        return "Data: ({},{},{},{},{},{})".format(self.id, self.location.place.city,
                                                  self.location.place.state,
                                                  self.location.place.country, self.conference,
                                                  self.year)
    ...
    def get_cost_CoolEffect(self, GLOB, cache, destination):
        logging.debug(
            "Estimating carbon footprint according to CoolEffect's methodology from {} to {}".
            format(self.location, destination))
        dist = self.location.get_distance(GLOB, cache, destination)
        if dist < 482:
    ...
    def get_thegoodtraveler(self, GLOB, cache, destination):
        logging.debug(
            "Estimating carbon footprint according to thegoodtraveler's methodology from {} to {}".
            format(self.location, destination))
        dist = self.location.get_distance(GLOB, cache, destination)
    ...
    def write_csv_row(self, writer):
        writer.writerow([
            self.id, self.location.place.city, self.location.place.state,
            self.location.place.country, self.location.continent, self.conference, self.year,
            self.footprint
        ])
    ...
    def get_cost_brighter(self, destination):
        ...
        try:
            ...
            print('Warning: failed to estimate cost from {} to {}'.format(
