...
class Location:
    ...
    def get_continent(self, GLOB, iso, gps):
        ...
        try:
            ...
            print(
                "Failed to find the continent associated to country {}".format(
                    str(e).split(":")[1].strip()
                )
            )
            return None
    ...
    def get_airport(self, GLOB, iso, gps):
        ...
        for d in data:
            if (
                d["type"] in ["medium_airport", "large_airport"]
                and d["country"] == name
            ):
                try:
...
class RawData:
    def __init__(
        ...
        self.location = Location(
            Place(city, state, country), iso, continent, GPS, airport
        )
    ...
    def get_cost_CoolEffect(self, GLOB, cache, destination):
        logging.debug(
            "Estimating carbon footprint according to CoolEffect's methodology from {} to {}".format(
    ...
    def get_thegoodtraveler(self, GLOB, cache, destination):
        logging.debug(
            "Estimating carbon footprint according to thegoodtraveler's methodology from {} to {}".format(
                self.location, destination
            )
        )
        dist = self.location.get_distance(GLOB, cache, destination)
    ...
    def get_cost_brighter(self, destination):
        print(
            "Estimating brighter cost from {} to {}".format(self.location, destination)
        )
        ...
        try:
            ...
            print(
                "Warning: failed to estimate cost from {} to {}".format(
                    self.location, destination
                )
            )
            cost = None
